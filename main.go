...
func homeLink(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		var tmpl = template.Must(template.New("home").ParseFiles("view.html"))
...
func readEvent(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		var tmpl = template.Must(template.New("read").ParseFiles("view.html"))
		var err = tmpl.Execute(w, nil)
	...
	if r.Method == "POST" {
		...
		var tmpl = template.Must(template.New("result_r").ParseFiles("view.html"))
		...
		var vector = r.FormValue("data_r")

		csr, err := db.Collection("student").Find(ctx, bson.M{"name": vector})
		if err != nil {
			log.Fatal(err.Error())
		}
		defer csr.Close(ctx)
		...
		angka := strconv.Itoa(grade)

		var data = map[string]string{"name": name, "grade": angka, "category": category}
...
func main() {
	...
	router.HandleFunc("/read", readEvent).Methods("GET")
	router.HandleFunc("/read", readEvent).Methods("POST")
	router.HandleFunc("/create", createEvent).Methods("GET")
